...
class PollWorker(val context: Context, workerParameters: WorkerParameters): Worker(context, workerParameters){
    override fun doWork(): Result{
        ...
        return Result.success()
        /* val lastBuildId = QueryPreferences.getLastResultId(context)
        val items: List<GalleryItem> = FlickrFetchr().fetchPhotosRequest().execute().body()?.photos?.galleryItems ?: emptyList()
        if(items.isEmpty()){
            return Result.success()
        }

        val resultId = items.first().id
        if(resultId== lastBuildId){
            Log.i(TAG, "Got an old result: $resultId")
        } else{
            Log.i(TAG, "Got a new result: $resultId")
            QueryPreferences.setLastResultId(context, resultId)
